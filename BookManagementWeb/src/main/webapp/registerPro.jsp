...
try { // 모든 파라미터 가져오기 및 유효성 검사를 포함하는 메인 try-catch 블록 시작.
    ...
    String sBookCode = request.getParameter("bookCode");    // 도서 코드 파라미터를 String으로 가져옴.
    ...
    String sPrice = request.getParameter("price");          // 가격 파라미터를 String으로 가져옴.
    ...
    int bookCode = 0; // 도서 코드 변수 초기화.
    if (errorMessage == null) { // 이전 단계에서 오류가 없으면 진행.
        try {
            bookCode = Integer.valueOf(sBookCode); // 도서 코드를 정수로 변환.
        } catch (NumberFormatException e) {
            errorMessage = "도서 코드가 올바르지 않습니다."; // 숫자가 아닐 경우 오류 메시지.
        }
    }
    ...
    if (errorMessage == null) { // 이전 단계에서 오류가 없으면 진행.
        try {
            price = Integer.valueOf(sPrice); // 가격을 정수로 변환.
        } catch (NumberFormatException e) {
    ...
    if (errorMessage == null) { // 이전 단계에서 오류가 없으면 진행.
        ...
        try {
            ...
            errorMessage = "날짜 형식이 올바르지 않습니다. ex)2024-mm-dd"; // 형식 불일치 시 오류 메시지.
        }
    }
    ...
    if (errorMessage == null) { // 오류가 없는 경우.
        // 모든 유효성 검사를 통과한 경우: 도서 추가 및 성공 알림.
        dao.addBook(bookCode, bookName, bookWriter, publisher, price, date); // 도서 정보를 추가하는 기능.
        %>
        <script type="text/javascript">
            alert("도서등록이 완료되었습니다."); // 성공 알림 메시지.
